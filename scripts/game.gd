...
func _layout_ui() -> void:
	if pause_button != null:
		pause_button.position = Vector2(size.x - 206.0, 34.0)

	if state_name == "title":
		if exit_button != null:
			exit_button.position = Vector2(92.0, 446.0)
	elif state_name == "paused":
		if resume_button != null:
			resume_button.position = Vector2(size.x * 0.5 - 182.0, size.y * 0.5 + 34.0)
		if exit_button != null:
			exit_button.position = Vector2(size.x * 0.5 + 12.0, size.y * 0.5 + 34.0)
	elif state_name == "game_over":
		if exit_button != null:
			exit_button.position = Vector2(size.x * 0.5 - 85.0, size.y * 0.5 + 106.0)
...
func _draw_board() -> void:
	...
	if state_name == "title":
		draw_rect(board, Color(0, 0, 0, 0.2), true)
		var preview_tag := Rect2(Vector2(board.end.x - 228.0, board.position.y - 12.0), Vector2(204.0, 42.0))
		_draw_panel(preview_tag, Color("132844", 0.82), _with_alpha(Color("8dfcff"), 0.32))
		_draw_label(preview_tag.position + Vector2(18.0, 28.0), "LIVE BOARD PREVIEW", 16, Color("f2fdff"))
...
func _draw_overlay() -> void:
	if state_name == "title":
		var marquee := Rect2(Vector2(62.0, 58.0), Vector2(540.0, 372.0))
		_draw_panel(marquee, Color("4d1645", 0.82), _with_alpha(Color("ffd36b"), 0.5))
		draw_rect(Rect2(marquee.position + Vector2(18.0, 18.0), Vector2(marquee.size.x - 36.0, 12.0)), _with_alpha(Color("fff7c8"), 0.22), true)
		var badge := Rect2(marquee.position + Vector2(24.0, 26.0), Vector2(144.0, 34.0))
		_draw_panel(badge, Color("102a47", 0.82), _with_alpha(Color("8dfcff"), 0.28))
		_draw_label(badge.position + Vector2(18.0, 23.0), "ARCADE DROP", 14, Color("f0fdff"))
		_draw_shadowed_label(marquee.position + Vector2(30.0, 126.0), "DopaQiX", 88, Color("53113f"), Color("fff1cb"))
		_draw_shadowed_label(marquee.position + Vector2(38.0, 178.0), "Native", 42, Color("1a4d68"), Color("8cf8ff"))
		_draw_label(marquee.position + Vector2(30.0, 230.0), "Slice the candy glow, slam the rail shut,", 23, Color("fff4da"))
		_draw_label(marquee.position + Vector2(30.0, 260.0), "and steal the whole floor before the sparks do.", 23, Color("fff4da"))
		var high_badge := Rect2(marquee.position + Vector2(30.0, 288.0), Vector2(168.0, 40.0))
		_draw_panel(high_badge, Color("0f2742", 0.86), _with_alpha(Color("7defff"), 0.28))
		_draw_label(high_badge.position + Vector2(18.0, 27.0), "HI %08d" % high_score, 17, Color("effdff"))
		var level_badge := Rect2(marquee.position + Vector2(210.0, 288.0), Vector2(132.0, 40.0))
		_draw_panel(level_badge, Color("6c2a33", 0.82), _with_alpha(Color("ffd981"), 0.26))
		_draw_label(level_badge.position + Vector2(18.0, 27.0), "%d LEVEL RUN" % FINAL_LEVEL, 17, Color("fff3cf"))
		var cta_fill := Color("ffd56d")
		cta_fill = cta_fill.lerp(Color("fff2a1"), 0.18 + sin(title_phase * 4.0) * 0.08)
		var cta := Rect2(marquee.position + Vector2(30.0, 340.0), Vector2(368.0, 54.0))
		_draw_panel(cta, cta_fill, _with_alpha(Color("ff5ea4"), 0.52))
		...
		_draw_centered_label(Vector2(modal.get_center().x, modal.position.y + 60.0), "CRASH", 42, Color("fff0dc"))
		_draw_centered_label(Vector2(modal.get_center().x, modal.position.y + 104.0), status_message, 18, Color("ffd0c0"))
	elif state_name == "game_over":
		var modal := Rect2(Vector2(size.x * 0.5 - 264.0, size.y * 0.5 - 122.0), Vector2(528.0, 244.0))
		_draw_panel(modal, Color("09101b", 0.84), _with_alpha(_theme_color("rail"), 0.22))
		if run_won:
			_draw_centered_label(Vector2(modal.get_center().x, modal.position.y + 72.0), "SYSTEM OVERLOADED", 44, Color("fff7d8"))
			_draw_centered_label(Vector2(modal.get_center().x, modal.position.y + 118.0), "You own the board.", 22, Color("dffcff"))
		else:
			_draw_centered_label(Vector2(modal.get_center().x, modal.position.y + 72.0), "GAME OVER", 44, Color("fff7d8"))
